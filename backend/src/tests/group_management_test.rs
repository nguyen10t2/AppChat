...
mod tests {
    ...
    use crate::modules::conversation::repository_pg::{ConversationPgRepository, ParticipantPgRepository};
    ...
    use crate::ENV;
    use crate::modules::conversation::model::{UpdateGroupRequest, AddMemberRequest};
    ...
    fn build_services(pool: sqlx::PgPool) -> (ConversationSvc, FriendSvc) {
        ...
        let friend_svc = FriendService::with_dependencies(
            friend_repo,
            user_repo,
        );
    ...
    async fn test_group_management_flow() {
        ...
        // Seed group
        sqlx::query("INSERT INTO conversations (id, type) VALUES ($1, 'group')").bind(group_id).execute(&pool).await.unwrap();
        ...
        sqlx::query("INSERT INTO participants (conversation_id, user_id, unread_count) VALUES ($1, $2, 0)").bind(group_id).bind(creator_id).execute(&pool).await.unwrap();
        ...
        let req = test::TestRequest::patch()
            ...
            .set_json(UpdateGroupRequest { name: Some("New Name".to_string()), avatar_url: None })
            .to_request();
        let resp = test::call_service(&app, req).await;
        assert_eq!(resp.status(), StatusCode::OK);

        // Verify in DB
        let name: String = sqlx::query_scalar("SELECT name FROM group_conversations WHERE conversation_id = $1").bind(group_id).fetch_one(&pool).await.unwrap();
        ...
        // Seed friendship
        sqlx::query("INSERT INTO friends (user_id1, user_id2, status) VALUES ($1, $2, 'ACCEPTED')").bind(creator_id).bind(member_id).execute(&pool).await.unwrap();
        ...
        let req = test::TestRequest::delete()
            .uri(&format!("/api/conversations/{}/members/{}", group_id, member_id))
            .insert_header(("Authorization", format!("Bearer {}", token)))
            .to_request();
        let resp = test::call_service(&app, req).await;
        assert_eq!(resp.status(), StatusCode::OK);
        ...
        // Cleanup
        let _ = sqlx::query("DELETE FROM group_conversations WHERE conversation_id = $1").bind(group_id).execute(&pool).await;
        let _ = sqlx::query("DELETE FROM participants WHERE conversation_id = $1").bind(group_id).execute(&pool).await;
        let _ = sqlx::query("DELETE FROM conversations WHERE id = $1").bind(group_id).execute(&pool).await;
        let _ = sqlx::query("DELETE FROM friends WHERE user_id1 = $1").bind(creator_id).execute(&pool).await;
        let _ = sqlx::query("DELETE FROM users WHERE id IN ($1, $2, $3)").bind(creator_id).bind(member_id).bind(outsider_id).execute(&pool).await;
    }
}
